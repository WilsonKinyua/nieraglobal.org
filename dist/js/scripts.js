(function ($) {
	 ...
	 * --------------------------------------------------------------------------- */
		...
		$('.tooltip-img').hover(function () {
			$(this).find('.tooltip-content').stop(true, true).show();
		}, function () {
			$(this).find('.tooltip-content').stop(true, true).hide();
		});
		 ...
		 * niceScroll
		 * --------------------------------------------------------------------------- */
		if ($('body').hasClass('nice-scroll-on') && $(window).width() > 767 && !navigator.userAgent.match(/(Android|iPod|iPhone|iPad|IEMobile|Opera Mini)/)) {
			$('html').niceScroll({
				autohidemode: false,
				cursorborder: 0,
		 ...
		 * WP Gallery
		 * --------------------------------------------------------------------------- */
		$('.gallery-icon > a').wrap('<div class="image_frame scale-with-grid"><div class="image_wrapper"></div></div>').prepend('<div class="mask"></div>').attr('rel', 'prettyphoto[gallery]').attr('data-rel', 'prettyphoto[gallery]').children('img').css('height', 'auto').css('width', '100%');
		...
		function mfn_jPlayer() {
			$('.mfn-jplayer').each(function () {
				...
				if ($('#training-home-section').length > 0) {
					var videoContainerHeight = 520;
				} else {
					var videoContainerHeight = Math.round($('#' + $(this).closest('.mfn-jcontainer').attr('id')).width() * (9 / 16));
				}
				$(this).jPlayer({
					ready: function () {
